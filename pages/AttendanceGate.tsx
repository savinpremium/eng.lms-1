...
import { UserCheck, History, RotateCcw, Loader2, ScanQrCode, AlertTriangle, Check, X, Zap, Camera, ShieldAlert, Image as ImageIcon, Smartphone } from 'lucide-react';
...
const AttendanceGate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!secure || !hasMedia) {
      setCameraError("Security Block: Live video requires HTTPS.");
    }
    ...
    if (isScannerActive) {
      const initScanner = async () => {
        ...
        try {
          ...
          if (!Html5Qrcode) {
            setCameraError("Scanner module missing.");
          ...
          const config = { 
            ...
            aspectRatio: 1.0 // Enforce square output
          };
  ...
  const startScanner = () => {
    ...
    if (!isSecure && window.location.hostname !== 'localhost') {
      setCameraError("HTTPS required for live scan.");
      return;
    }
  ...
  const handleFileScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert("No clear QR code detected.");
    } finally {
    ...
    if (student) {
      ...
      try {
        ...
        if (isPaid) {
          ...
          audioService.speak(`Not Paid`); // Explicit voice feedback
  ...
  const undoLastEntry = async () => {
    ...
    if (!confirm("Reverse last log?")) return;
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-20 relative">
      <header className="text-center animate-in fade-in duration-700">
        <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase italic leading-none mb-3 text-white">Attendance</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className={`relative rounded-[3rem] md:rounded-[4rem] overflow-hidden border-8 transition-all duration-300 shadow-3xl bg-slate-900 min-h-[450px] md:min-h-[500px] flex flex-col items-center justify-center p-0 text-center ${
          ...
          {!isScannerActive ? (
              ...
              {!isSecure ? (
                <div className="bg-rose-500/10 border border-rose-500/30 p-6 rounded-[2.5rem] flex flex-col items-center gap-4 text-center max-w-sm mx-auto">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-rose-500">
                    HTTPS connection required for live video.
                  </p>
                </div>
              ) : (
                <div className="bg-slate-950 p-6 rounded-[2.5rem] border border-slate-800 shadow-inner flex items-center gap-4 text-left max-w-sm mx-auto">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    Scan Student PVC Card to grant institutional entry.
                  </p>
                </div>
              )}
              ...
              <div className="flex flex-col gap-4 w-full max-w-xs mx-auto">
                {isSecure && (
                  <button 
                    ...
                    className="bg-blue-600 hover:bg-blue-500 text-white px-8 py-5 rounded-3xl font-black uppercase text-xs tracking-[0.2em] transition-all shadow-2xl flex items-center justify-center gap-4 border-b-4 border-blue-800"
                  >
                    <ScanQrCode size={20} />
                    Open Live Lens
                  </button>
                ...
                <div className="relative">
                  ...
                  <button 
                    ...
                    className="w-full bg-slate-800 border-slate-700 text-slate-300 px-8 py-5 rounded-3xl font-black uppercase text-xs tracking-[0.2em] transition-all flex items-center justify-center gap-4 border-b-4 shadow-xl"
                  >
            ...
            <div className="relative w-full aspect-square md:aspect-auto md:h-[500px] bg-black">
        ...
        <div className="space-y-8 animate-in slide-in-from-right-8 duration-700">
          <div className="bg-slate-900/50 p-10 md:p-12 rounded-[3rem] border border-slate-800 shadow-2xl relative overflow-hidden">
            ...
            {lastStudent ? (
              <div className="animate-in zoom-in duration-500">
                ...
                <div className="flex items-center gap-3 text-emerald-500 font-black text-xs uppercase tracking-[0.3em] mb-8">
                   ...
                   {status === 'GRACE' ? 'FEE OVERDUE' : 'Access Granted'}
                </div>
                <button onClick={undoLastEntry} className="flex items-center gap-2 bg-slate-800 text-rose-500 px-6 py-3 rounded-full font-black uppercase text-[10px] border border-slate-700 hover:bg-rose-600 transition-all shadow-lg">
                  <RotateCcw size={14} />
                  Reverse Log
                </button>
              </div>
              ...
              <div className="py-6 opacity-20 text-center">
                ...
                <p className="text-2xl font-black text-slate-400 uppercase italic tracking-tighter">Ready to Verify</p>
              </div>
            )}
          </div>

          <div className="bg-slate-950 p-10 md:p-12 rounded-[3rem] border border-slate-900 shadow-3xl">
            <h3 className="text-[10px] font-black tracking-[0.5em] uppercase text-slate-600 mb-8 flex items-center gap-3">
